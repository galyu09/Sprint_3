...
class TestRegistration:
    ...
    def test_new_user_registration_sucsess(self, driver):
        ...
        WebDriverWait(driver, 3).until(EC.element_to_be_clickable(Locators.BUTTON_REG_ENTER)).click() # кнопка зарегестрироваться
        ...
        expected_name = WebDriverWait(driver, 3).until(EC.presence_of_element_located(Locators.NAME_FIELD)).get_attribute('value')
        expected_email = WebDriverWait(driver, 3).until(EC.presence_of_element_located(Locators.EMAIL_FIELD)).get_attribute('value')
    ...
    def test_new_user_registration_incorrect_password_faild(self, driver):
        ...
        driver.find_element(*Locators.NAME).send_keys(new_name) # имя в форме регистрации (вводим валидное)
        driver.find_element(*Locators.EMAIL).send_keys(new_email) # емайл в форме регистрации
        driver.find_element(*Locators.PASSWORD).send_keys(Constants.INCORRECT_PASSWORD) # НЕвалидный пароль в форме регистрации

        WebDriverWait(driver, 3).until(
            EC.element_to_be_clickable(Locators.BUTTON_REG_ENTER)).click()  # кнопка зарегестрироваться

        error_message = WebDriverWait(driver, 3).until(EC.visibility_of_element_located(Locators.ERROR_PASSWORD_MESSAGE)).text
